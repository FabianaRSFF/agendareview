...
def cadastro(request):
    ...
    if len(senha) < 6:
        messages.error(request, "Senha necessita 6 caracteres.")
        return render(request, 'accounts/cadastro.html')
    ...
    messages.success(request, 'Cadastro efetuado com sucesso. Agora faça login')
...
def dashboard(request):
    ...
    if len(descricao) < 5:
        messages.error(request, 'Descrição muito curta.')
        form = FormContato(request.POST)
        return render(request, 'accounts/dashboard.html', {'form': form})
